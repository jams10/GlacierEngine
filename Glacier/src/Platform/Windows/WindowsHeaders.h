...
#define _WIN32_WINNT 0x0A00 // 코드에서 지원하는 최소 운영 체제 버전을 지정.
#include <sdkddkver.h>      // 윈도우즈 버전에 대한 매크로들이 정의되어 있는 헤더 파일.

// Windows.h 헤더 파일에는 우리가 작성할 애플리케이션에서 사용하지 않을 API가 많이 들어있는데,
// 아래와 같이 여러 매크로 정의를 통해 특정 API들을 제외시켜서 컴파일 시간을 향상시킬 수 있디/
#define WIN32_LEAN_AND_MEAN // 암호화, DDE, RPC, 셸 및 Windows 소켓과 같은 API를 제외
//#define NOMINMAX          // Windows.h에 min, max 매크로가 정의되어 있는데, 이는 std::min, max와 충돌하기 때문에 제외해줌. 프로젝트 전처리에서 정의함.
                            // 전처리에 추가하지 않을 경우 assimp 라이브러리 파일을 include 하는 과정에서 컴파일 오류 발생.
