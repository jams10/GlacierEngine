...
namespace Glacier
{
		...
		: m_Count(count)
	{
		...
		THROWFAILED(DirectX11Device::GetDevice()->CreateBuffer(&bufferDesc, &indexBufferData, m_gpuBuffer.GetAddressOf()));
	}
	...
	{
		DirectX11Device::GetDeviceContext()->IASetIndexBuffer(m_gpuBuffer.Get(), DXGI_FORMAT_R32_UINT, 0);
	}

	void DirectX11IndexBuffer::Unbind() const
	{
		DirectX11Device::GetDeviceContext()->IASetIndexBuffer(nullptr, DXGI_FORMAT_R32_UINT, 0);
	}
}
